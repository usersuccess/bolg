        ...
        <div class="col-xs-10" >
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <?php
                    $sql = "select * from mr_post where post_type=0 order by add_time desc ";
                    $result = $pdo->query($sql);
                    while($row = $result->fetch(PDO::FETCH_ASSOC)):
                        ?>
                        <div class="panel panel-default">
                            <img src='<?php
                            $sql1="select * from mr_user where id={$row['user_id']}";
                            $re=$pdo->query($sql1);
                            ...
                            <div class="panel-footer">评论(
                                ...
                                <div style="float:right"><a href="fmanager.php?id=<?php echo $row['user_id'] ?>"><?php echo $row['username']?></a>&nbsp;&nbsp;<?php echo date("Y-m-d H:i:s",$row['add_time']) ?></div></div>
                        </div>
                        <?php
                    endwhile;
                    ?>
